...
namespace detail
{
    ...
    bool ZipCentralDirectoryFileHeader::Deserialize(std::istream& stream)
    {
        ...
        {
            stream.clear();
            stream.seekg(static_cast<std::ios::off_type>(static_cast<std::streamoff>(stream.tellg()) - stream.gcount()),
                         std::istream::beg);
            return false;
        }
        ...
        if (ExtraFieldLength > 0)
        {
            ...
            while (extraField.Deserialize(stream, extraFieldEnd)) { ExtraFields.push_back(extraField); }
        }
    ...
    void ZipCentralDirectoryFileHeader::Serialize(std::ostream& stream)
    {
        ...
        if (ExtraFieldLength > 0)
        {
            for (auto& extraField : ExtraFields) { extraField.Serialize(stream); }
        }
